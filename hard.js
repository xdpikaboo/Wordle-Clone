...
easy.textContent = "This is too hard!";
easy.addEventListener("click", () => (window.location.href = "index.html"));
buttons.append(easy);

const harder = document.createElement("button");
harder.textContent = "Still too ez~";
harder.addEventListener("click", () => (window.location.href = "harder.html"));
...
const checkRow = () => {
  if (currentRow >= 5) {
    showMessage(
      "Game Over! Words are " + wordle1 + "," + wordle2 + "," + wordle3
    );
  }
  ...
  if (
    currentTile > size - 1 &&
    (!isGameOver1 || !isGameOver2 || !isGameOver3)
  ) {
    fetch(
